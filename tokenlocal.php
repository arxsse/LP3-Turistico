<?php
// Permite sobrescribir la URL base con la variable de entorno TURISTICO_API_BASE si es necesario.
$apiBaseUrl = getenv('TURISTICO_API_BASE') ?: 'http://localhost:8080/api/v1/auth';
...
function callApi($url, array $payload)
{
	$curl = curl_init();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if ($action === 'register') {
		...
		$result = callApi($apiBaseUrl . '/register', $payload);
		...
		$result = callApi($apiBaseUrl . '/login', $payload);
...
<head>
	<meta charset="UTF-8">
	<title>Sistema Turístico - Registro y Token</title>
	<style>
		*,
		*::before,
		*::after {
...
<div class="page-shell">
	...
	<div class="surface">
		<div class="surface-header">
			<h1>Token Local</h1>
			<p>Gestiona el registro y la obtención de credenciales en dos acciones simples.</p>
		</div>
		...
		<div class="form-layout">
			<div class="form-card">
				<div>
					<h2>Registrar</h2>
					<p class="form-subtitle">Crea un nuevo acceso y recibe su token al instante.</p>
				</div>
			...
			<div class="form-card">
				<div>
					<h2>Obtener token</h2>
					<p class="form-subtitle">Inicia sesión para reutilizar el token vigente.</p>
				</div>
				<form method="post" class="form-stack">
